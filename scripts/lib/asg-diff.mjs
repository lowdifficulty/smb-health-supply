import { claimStableKey, round2 } from './asg-parse.mjs'
...
function formatClaimLabel(c) {
  return `${c.patientName} · DOS ${c.dos}${c.submissionId ? ` · ${c.submissionId}` : ''}`
}

/**
 * Compare two snapshots. Returns null if first run (no previous).
 */
...
export function formatDiffReport(diff) {
  const lines = []
  lines.push(`ASG sheet check — ${diff.summary}`)
  ...
  if (diff.totalsDelta) {
    ...
    lines.push(
      `  Billed: ${formatMoneyDelta(diff.totalsDelta.billedDollars)}`,
    )
    lines.push(
      `  Primary remit: ${formatMoneyDelta(diff.totalsDelta.primaryRemitDollars)}`,
    )
    lines.push(
      `  Secondary remit: ${formatMoneyDelta(diff.totalsDelta.secondaryRemitDollars)}`,
    )
    lines.push(
      `  Units: ${diff.totalsDelta.unitsBilled >= 0 ? '+' : ''}${diff.totalsDelta.unitsBilled}`,
    )
  }
